...
public class OnvifExecutor {
    ...
    private void performXmlRequest(OnvifDevice device, OnvifRequest request, Request xmlRequest) {
        ...
        client.newCall(xmlRequest)
                .enqueue(new Callback() {
                    ...
                    @Override
                    public void onResponse(@NonNull Call call, @NonNull Response xmlResponse) throws IOException {
    ...
    private Request buildOnvifRequest(OnvifDevice device, OnvifRequest request) {
        return new Request.Builder().url(getUrlForRequest(device, request)).addHeader("Content-Type", "text/xml; charset=utf-8").post(reqBody).build();
    }
